    ...
    public class VagasDao : Dao
    {
        ...
        public Vagas Alterar(Vagas vagas, int id = 0)
        {
            try
            {
                ...
                string sql = "UPDATE tb_vaga SET idempresa = @idempresa, idturno = @idturno, idmodalidade = @idmodalidade, idescolaridade = @idescolaridade, titulo_vaga = @titulo_vaga, descricao = @descricao, salario = @salario, data_limite_inscricao = @data_limite_inscricao, quantidade_vaga = @quantidade_vaga  WHERE id = @id";
